...
void ST7789_DrawDiagonalPlatform(uint16_t x1, uint16_t y1, uint16_t width, uint16_t height, uint16_t slope, uint16_t color) {
   int16_t y2 = y1 + (width * slope / 100); // Calculate end y based on slope percentage
   
   // Draw main diagonal lines
   ST7789_DrawLine(x1, y1, x1 + width, y2, color);  // Bottom edge
   ST7789_DrawLine(x1, y1 + height, x1 + width, y2 + height, color);  // Top edge
   
   // Draw connecting vertical lines
   ST7789_DrawLine(x1, y1, x1, y1 + height, color);  // Left edge  
   ST7789_DrawLine(x1 + width, y2, x1 + width, y2 + height, color);  // Right edge
}

void ST7789_WriteChar(uint16_t x, uint16_t y, char c, uint16_t color) {
    ...
    const CharacterDef* charDef = GetCharacterDef(c);

    // Draw each pixel in the character
    for(int i = 0; i < charDef->num_pixels; i++) {
        uint8_t px = charDef->pixels[i][0];
        uint8_t py = charDef->pixels[i][1];
        ST7789_DrawPixel(x + px, Y_MAX - (y + py), color);
    }
}

void ST7789_WriteString(uint16_t x, uint16_t y, const char* str, uint16_t color) {
    while(*str) {
        ST7789_WriteChar(x, y, *str++, color);
...
// Function to write score with leading zeros
void ST7789_WriteScore(uint16_t x, uint16_t y, uint32_t score, uint16_t color) {
   char scoreStr[7];
   snprintf(scoreStr, sizeof(scoreStr), "%03lu", score); // Format with leading zeros
   ST7789_WriteString(x, y, scoreStr, color);
}
/********************************Public Functions***********************************/
